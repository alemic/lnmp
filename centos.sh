...
if [ -s tengine-1.5.1.tar.gz ]; then
  echo "tengine-1.5.1.tar.gz [found]"
  else
  echo "Error: tengine-1.5.1.tar.gz not found!!!download now......"
  wget -c http://tengine.taobao.org/download/tengine-1.5.1.tar.gz
fi
...
./configure --prefix=/data/lnmp/mysql --with-extra-charsets=complex --enable-thread-safe-client --enable-assembler --with-mysqld-ldflags=-all-static --with-charset=utf8 --enable-thread-safe-client --with-big-tables --with-readline --with-ssl --with-embedded-server --enable-local-infile --with-plugins=innobase
else
./configure --prefix=/data/lnmp/mysql --with-extra-charsets=complex --enable-thread-safe-client --enable-assembler --with-mysqld-ldflags=-all-static --with-charset=utf8 --enable-thread-safe-client --with-big-tables --with-readline --with-ssl --with-embedded-server --enable-local-infile
fi
make && make install
cd ../
...
cp /data/lnmp/mysql/share/mysql/my-medium.cnf /etc/my.cnf
sed -i 's/skip-locking/skip-external-locking/g' /etc/my.cnf
if [ $installinnodb = "y" ]; then
sed -i 's:#innodb:innodb:g' /etc/my.cnf
fi
/data/lnmp/mysql/bin/mysql_install_db --user=mysql
chown -R mysql /data/lnmp/mysql/var
chgrp -R mysql /data/lnmp/mysql.
cp /data/lnmp/mysql/share/mysql/mysql.server /etc/init.d/mysql
...
ln -s /data/lnmp/mysql/lib/mysql /usr/lib/mysql
ln -s /data/lnmp/mysql/include/mysql /usr/include/mysql
/etc/init.d/mysql start

ln -s /data/lnmp/mysql/bin/mysql /usr/bin/mysql
ln -s /data/lnmp/mysql/bin/mysqldump /usr/bin/mysqldump
ln -s /data/lnmp/mysql/bin/myisamchk /usr/bin/myisamchk
ln -s /data/lnmp/mysql/bin/mysqld_safe /usr/bin/mysqld_safe

/data/lnmp/mysql/bin/mysqladmin -u root password $mysqlrootpwd
...
/data/lnmp/mysql/bin/mysql -u root -p$mysqlrootpwd -h localhost < /tmp/mysql_sec_script
...
cd mysql-5.5.28/
cmake -DCMAKE_INSTALL_PREFIX=/data/lnmp/mysql -DEXTRA_CHARSETS=all -DDEFAULT_CHARSET=utf8 -DDEFAULT_COLLATION=utf8_general_ci -DWITH_READLINE=1 -DWITH_SSL=system -DWITH_ZLIB=system -DWITH_EMBEDDED_SERVER=1 -DENABLED_LOCAL_INFILE=1
make && make install
